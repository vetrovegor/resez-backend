...
class NotifyController {
    async sendNotifies(
        req: RequestWithBodyAndUser<SendNotifiesDTO>,
        res: Response,
        next: NextFunction
    ) {
        try {
    ...
    async getUserUnreadNotifiesCount(
        req: RequestWithUser,
        res: Response,
        next: NextFunction
    ) {
        try {
            const count = await notifyService.getUserUnreadNotifiesCount(
                req.user.id
            );
    ...
    async getUserNotifies(
        req: RequestWithQueryAndUser<PaginationQuery & UserNotifyQuery>,
        res: Response,
        next: NextFunction
    ) {
        try {
            ...
            const data = await notifyService.getUserNotifies(
                req.user.id,
                limit,
                offset,
                unread
    ...
    async readNotify(
        ...
        try {
            const notify = await notifyService.readNotify(
                req.user.id,
                req.params.id
            );
    ...
    async readAllNotifies(
        ...
        try {
            ...
            const data = await notifyService.readAllNotifies(
                req.user.id,
                limit,
                offset
            );
