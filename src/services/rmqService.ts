...
class RmqService {
    ...
    async consumeQueues() {
        ...
        this.channel.consume('get-verified-user-by-telegram-chat-id', async msg => {
            const { telegramChatId } = JSON.parse(msg.content.toString());

            const user = await userService.getVerifiedUserByTelegramChatId(telegramChatId);
        ...
        this.channel.consume('verify-user', async msg => {
            const { userId, telegramChatId } = JSON.parse(msg.content.toString());
